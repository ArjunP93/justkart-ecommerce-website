...
module.exports={
    getHome:async(req,res)=>{
        let bannerView = await userhelpers.bannerPush()
        console.log('bannersssss',bannerView);
        
        if(loginStatus){
            res.render('user/userhome',{loginheader:true,bannerView})
        }else{
            res.render('user/userhome',{loginheader:false,bannerView})
        }
    },
    showLogin:(req,res)=>{
    ...
    shopView:async(req,res)=>{
        ...
        userhelpers.listProductShop(page,perPage).then((response)=>{
            ...
            adminhelpers.findAllcategories().then((cat)=>{
                if(req.session.userIn)
              {  res.render('user/shop',{response,cat,loginheader:true,pages:Math.ceil(orderListCount/perPage)})}
              else{
                res.render('user/shop',{response,cat,pages:Math.ceil(orderListCount/perPage)})
              }
            })
    ...
    productPage:(req,res)=>{
        userhelpers.viewProductDetails(req.params.id).then((response)=>{
            console.log(response);
    ...
    categoryFiltering:async(req,res)=>{
        ...
        console.log('catname',categoryName);
         ...
         const page= req.query.page || 1;
         const perPage =10;
         const count = await db.products.find({category:categoryName}).countDocuments({})
         console.log('count',count);
         const orderListCount = count;
        ...
        await userhelpers.findAllProducts(categoryName,page,perPage).then((response)=>{
    ...
    postotp:(req,res)=>{
        userhelpers.otpLogin(req.body.mobile).then((data)=>{
        console.log(data,'daatadadsfdskfjkdsfsjd');
     ...
     ajaxAddToCart:(req,res)=>{
        ...
        userhelpers.addToCart_post(productId,userId,qty,subt).then((response)=>{
     ...
     ajaxViewCart:async(req, res)=>{
        ...
        userhelpers.viewUser_cart(userId).then((response)=>{

            console.log("view:",response);
            let count= response.count

            console.log('response for view cart',response);
            res.render('user/cart',{response,count,userId,loginheader:true,total})
    ...
    deleteInCart:async(req,res)=>{
        ...
        console.log('loggggggg_test');
        let userId= req.session.user._id
        await userhelpers.deleteProInCart(req.query.proId,userId)
            res.json(true)
    ...
    getwishlist:async(req,res)=>{
        ...
        const userId = req.session.user._id
        userhelpers.viewWishlist(userId).then((response)=>{

            res.render('user/wishlist',{loginheader:true,response})
        })
    },
    ...
    checkout:async(req,res)=>{
        ...
        console.log('total',total_checkout);

    
        res.render('user/checkout',{address,cart,total_checkout,loginheader:true})
    },
    post_checkout:async(req,res)=>{
        ...
        if(req.body.couponCode=="N/A"){
           ...
           total_checkout=total_Aggregate[0].total
            ...
            console.log('totalcheouuuurt inside false',total_checkout);
        }
         console.log('totaaall outside',total_checkout);
        
        let selectedAddress = await userhelpers.getSelectedAddress(req.body.address,userid)

        // console.log('cart',cart );
        // console.log('total_chekout',total_checkout);
        ...
        let payStatus
        if(req.body.paymentMethod==="online"||req.body.paymentMethod==="wallet"){
            payStatus = "pending"
        }else{
            payStatus = "success"
        }
        ...
        await userhelpers.createOrder(userid,userInfo,cart,req.body.paymentMethod,couponCode,payStatus,total_checkout,shippingAddressObj).then((orderId)=>{
          let response={}
           ...
           if(req.body.paymentMethod==='COD'){
            ...
            //wallet payment
                ...
                if(result == null){
                    ...
                    userhelpers.purchaseWithWallet(userid,orderId,parseInt(amountToPay),parseInt(walletbalance)).then(()=>{
                        response.wallet=true
    ...
    get_orderDetails:(req,res)=>{
        ...
        // console.log('ahstesereryejhwjh');

        userhelpers.userOrderDetails(orderId).then((currentOrder)=>{
            // console.log('currentOrder',currentOrder);
            res.render('user/orderdetail',{currentOrder,loginheader:true})
        })


    },
    post_cancel_order:(req,res)=>{
    ...
    get_userOrdersExpand:(req,res)=>{
       let orderId = req.params.id
        userhelpers.expandUserOrders(orderId).then((orderView)=>{
            console.log('orders',orderView);
            res.render('user/viewOrderDetail',{orderView,loginheader:true})
        })
...
const page= req.query.page || 1;
          const perPage =10;
          const count = await orderSchema.order.countDocuments({userid:userId})
          console.log('count',count);
          const orderListCount = count;
       ...
       let addressCount
       if(multipleAddress){ addressCount = multipleAddress.Address.length}
       else{
         addressCount = 0 
       }
      ...
      console.log('orderssssss',orders);
       ...
       res.render('user/profile',{loginheader:true,multipleAddress ,addressCount,orders,pages:Math.ceil(orderListCount/perPage)})
